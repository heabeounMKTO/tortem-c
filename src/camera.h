...
static inline CameraSettings* new_camera_settings(int width, int height, double focal_length, double viewport_height, double vfov, Vec3_d look_from, Vec3_d look_at){
  ...
  Vec3_d pixel_delta_v = vec3d_div(viewport_v, vec3d_from_int(cam->height));
    Vec3_d viewport_upper_left = vec3d_sub(
      vec3d_sub(
          vec3d_sub(
              cam->look_from,
              vec3d_mul(vec3d_from_float(cam->focal_length), w)
          ),
