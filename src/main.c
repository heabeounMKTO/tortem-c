...
int main() {
  ...
  Material sphere_m_glass2 = new_dialectric_mat(vec3d_new(1.0, 1.0, 1.0), (1.0/1.25));
  ...
  Sphere sphere_big = new_sphere(vec3d_new(0.0,0.05,-1.0), sphere_m_fuzz, 0.5);
  Sphere sphere_smol = new_sphere(vec3d_new(0.0,0.05,-1.0), sphere_m_glass2, 0.4);
  ...
  Vec3_d look_from = vec3d_new(-2.0, 2.0, -2.0);
  Vec3_d look_at = vec3d_new(-2.5,2.0,-2.4);
  CameraSettings *cam = new_camera_settings(IMAGE_WIDTH, IMAGE_HEIGHT, 1.0, 2.0,
                                             look_from, 90, look_from, look_at);
